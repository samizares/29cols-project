...
class GalleryController extends BaseController
{
  ...
  public function __construct () {
    ...
    public function index(User $user)
    {
        ...
        return View::make('layout.galleries',compact('galleries'));
    }

    public function latest()
    {
        $bosts = Blog::orderBy('id','desc')->paginate(10);
        $this->layout->title = 'Latest Posts';
        $this->layout->main = View::make('hhome')->nest('content','posts.latest',compact('posts'));
    }

    public function showPost(Post $post)
    {
        $comments = $post->comments()->where('Is_approved', '=', 1)->get();
